...
"use strict";
let url_maker;
// for each block under each webcomic's title (paragraph with url format, or list of comics)
for (let elem of document.querySelector('.entry').children) {
    // if current block is paragraph with an url to format
    if (elem.tagName.toUpperCase()==='P' && elem.childElementCount === 1 && elem.children[0].tagName.toUpperCase()==='A') {
        let url = elem.children[0].text;
        url_maker = function(m0, m1) { return '<a href="'+ url.replace(/{[^{}]*}/, m1) +'">'+m1+'</a>'; };
    // if current block is a list of comics
    } else if (elem.tagName.toUpperCase()==='UL') {
        // create link if necessary for every comic
        for (let bullet of elem.children) {
            const bullet_innerHTML = bullet.innerHTML;
            let to_linkify;
            let the_rest = "";
            let matching = bullet_innerHTML.match(/(<code>.*<\/code>):\s/);
            //console.log(matching);
            // detecting if current bullet contains a code for an url followed by an explanation (1231: something),
            // or if it is a list of codes
            if (matching !== null && matching.constructor === Array) {
                to_linkify = matching[1];
                the_rest = bullet_innerHTML.match(/:\s.*/)[0];
            } else {
                to_linkify = bullet_innerHTML;
            }
            //console.log(to_linkify);
            let linkified = to_linkify.replace(/<code>([^<>]*)<\/code>/gi, url_maker);
            //console.log(linkified);
            bullet.innerHTML = linkified + the_rest;
        }
    }
}
</script>
